...
two are mirrored **by hand**. Nothing machine-compares them any more: the lockstep tool and
its required status check were both removed in the 2026-08-04 harness rebuild, so an
unmirrored edit here turns nothing red and only a careful author keeps the pair honest.
