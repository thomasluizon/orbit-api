    ...
    public async Task<Result<CheckoutResponse>> Handle(CreateCheckoutCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(user.ReferralCouponId))
            {
                ...
                logger.LogInformation("Applying referral coupon {CouponId} to checkout for user {UserId}", user.ReferralCouponId, request.UserId);
            }
            else
            {
            ...
            logger.LogInformation("Checkout created for user {UserId} price={PriceId} country={Country}", request.UserId, priceId, countryCode);
            return Result.Success(new CheckoutResponse(session.Url));
        }
        catch (StripeException ex)
        {
            logger.LogError(ex, "Stripe API error during checkout for user {UserId}", request.UserId);
