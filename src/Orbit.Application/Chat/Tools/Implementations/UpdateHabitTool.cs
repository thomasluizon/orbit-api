    ...
    IGenericRepository<Habit> habitRepository) : IAiTool
{
    ...
    public object GetParameterSchema() => new
    {
        ...
        properties = new
        {
            ...
            icon = new { type = JsonSchemaTypes.String, description = "New emoji icon (e.g. '🏃'). Set to null to clear. Max 32 characters.", nullable = true },
            frequency_unit = new
            {
    ...
    private static HabitUpdateParams ResolveUpdateParams(JsonElement args, Habit habit)
    {
        ...
        var icon = ResolveIcon(args, habit);
    ...
    private static string? ResolveIcon(JsonElement args, Habit habit) =>
        JsonArgumentParser.PropertyExists(args, "icon")
            ? JsonArgumentParser.GetNullableString(args, "icon")
            : habit.Icon;
