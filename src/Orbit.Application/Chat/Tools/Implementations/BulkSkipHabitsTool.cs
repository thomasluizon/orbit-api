    ...
    BulkHabitReplayPlanner replayPlanner) : IAiTool
{
    ...
    public async Task<ToolResult> ExecuteAsync(JsonElement args, Guid userId, CancellationToken ct)
    {
        ...
        var plan = await replayPlanner.GetOrCreateAsync(userId, typeof(BulkSkipHabitsCommand).FullName!, async token =>
        {
        ...
        return await BulkUpdateHabitsTool.ExecuteInChunksAsync(
            plan.HabitIds.Select(id => new BulkSkipItem(id, plan.Date)).ToList(),
