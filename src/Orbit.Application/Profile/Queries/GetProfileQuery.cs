...
public record ProfileResponse(string Name, string Email, string? TimeZone, bool AiMemoryEnabled, bool AiSummaryEnabled);
    ...
    IGenericRepository<User> userRepository) : IRequestHandler<GetProfileQuery, ProfileResponse>
{
    public async Task<ProfileResponse> Handle(GetProfileQuery request, CancellationToken cancellationToken)
    {
        ...
        return new ProfileResponse(user.Name, user.Email, user.TimeZone, user.AiMemoryEnabled, user.AiSummaryEnabled);
    }
}
