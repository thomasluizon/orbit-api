...
public class GlobalRulesSection : IPromptSection
{
    ...
    public string Build(PromptContext context)
    {
        ...
        sb.AppendLine("""
            ...
            15. HABIT EMOJIS: When creating a habit or sub-habit, set a concise relevant emoji if the activity clearly suggests one. Use the exact emoji when the user requests a specific emoji. When the user asks to make all habit emojis sensible, call query_habits as needed, then call update_habit once per habit with an emoji that matches each habit's title or description. Do not change titles, schedules, or other fields unless requested.
