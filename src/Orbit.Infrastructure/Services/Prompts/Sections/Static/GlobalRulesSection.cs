...
public class GlobalRulesSection : IPromptSection
{
    ...
    public string Build(PromptContext context)
    {
        ...
        sb.AppendLine("""
            ...
            4. DUPLICATE PREVENTION: Before creating a habit or goal, silently check the indexes already provided above. Those indexes are for ACTIVE duplicate checks. A COMPLETED parent may still appear only to preserve the path to an active sub-habit, but COMPLETED entries never block creating a new habit/goal with the same or similar name. If a very similar ACTIVE habit or goal already exists, mention it briefly and ask if they want a new one or to update the existing. If no ACTIVE duplicate exists, create it immediately. NEVER ask the user for permission to check for duplicates - you already have the list.
            ...
            18. STRICT MATCH FOR LOG / COMPLETE / SKIP: When the user reports doing, finishing, or completing an activity, only call log_habit, bulk_log_habits, skip_habit, or status-change tools on habits whose title clearly corresponds to the words the user actually used (same activity, including obvious translations such as "meditei" -> "Meditate" / "Meditar"). Do NOT log or complete a habit just because it shares a tag, parent, time-of-day, routine, or general theme with something the user mentioned. If the user mentions activity "X" and NO habit in the index above clearly matches "X", do NOT substitute a related habit. Instead, tell the user briefly that you don't see a matching habit and ask if they want to create one. The same rule applies to the user mentioning multiple activities: log exactly the habits they named, no more, no fewer.
            """);
        return sb.ToString();
    }
}
