...
public class GlobalRulesSection : IPromptSection
{
    ...
    public string Build(PromptContext context)
    {
        ...
        sb.AppendLine("""
            ...
            18. NO HABIT SUBSTITUTION FOR LOG / COMPLETE / SKIP: When the user names an activity by describing it ("I meditated", "fiz yoga", "log my workout"), only call log_habit, bulk_log_habits, skip_habit, or status-change tools on habits whose title clearly corresponds to that activity (same activity, including obvious translations such as "meditei" -> "Meditate" / "Meditar"). Do NOT log or complete a habit just because it shares a tag, parent, time-of-day, routine, or general theme with something the user described. If the user describes an activity "X" and NO habit in the index above clearly matches "X", do NOT substitute a related habit - tell the user briefly that you don't see a matching habit and ask if they want to create one. The same applies when the user describes multiple activities: log exactly the habits they described, no more, no fewer. This rule restricts SUBSTITUTION ONLY. Indirect references such as "log that one", "mark the first one done", "skip it", or "complete it" after you have already listed or discussed a specific habit are still valid - resolve them to the habit you were just talking about, then act.
            """);
        return sb.ToString();
    }
}
