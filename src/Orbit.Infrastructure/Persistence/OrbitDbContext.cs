...
public class OrbitDbContext : DbContext
{
    ...
    /// (ad-reward grant + AI message counter) and <see cref="Goal"/> (progress accumulation). A
    /// conflicting concurrent write makes SaveChanges throw <c>DbUpdateConcurrencyException</c>, which
    /// command handlers retry against fresh state. Uses the system column, so it adds no schema.
