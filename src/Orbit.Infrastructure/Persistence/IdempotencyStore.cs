...
{
    public async Task<string?> FindResponseBodyAsync(Guid userId, string idempotencyKey, CancellationToken cancellationToken)
    {
        return await context.ProcessedRequests
            .AsNoTracking()
            .Where(request => request.UserId == userId && request.IdempotencyKey == idempotencyKey)
            .Select(request => request.ResponseBody)
            .FirstOrDefaultAsync(cancellationToken);
    }

    public IIdempotencyReservation Reserve(Guid userId, string idempotencyKey)
    {
        var record = ProcessedRequest.Create(userId, idempotencyKey);
